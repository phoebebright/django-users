...
from .permission_mixins import UserCanAdministerMixin, IsAdministrator
...
from .models import CustomUser, Organisation, Role, VerificationCode, CommsChannel
...
class CommsChannelRateThrottle(SimpleRateThrottle):
    ...
    def allow_request(self, request, view):
        ...
        if not is_allowed:
            ...
            messages.add_message(request, messages.WARNING, "You have exceeded the request limit. Please try again in a minute.")
        ...
        if email:
            ...
            try:
                ...
                for item in django_user.comms_channels.all():
                    channels.append({'channel_id': item.pk, 'channel_type': item.channel_type, 'email': item.obfuscated_email, 'mobile':item.obfuscated_mobile, 'verified': item.is_verified})
...
class CommsChannelViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        return Response({'next': reverse('users:verify_channel', args=[channel.pk]) + '?next='+back}, status=status.HTTP_200_OK)
...
class OrganisationViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.request.query_params.get('default_authority', None) and self.request.query_params.get('default_authority') == '0':
            # remove from filter
