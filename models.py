...
from skorie.common.models import OrganisationBase, PersonOrganisationBase, PersonBase, RoleBase, ModelRoles, \
    DataQualityLogBase
...
class CustomUserBase(AbstractBaseUser, PermissionsMixin, DataQualityMixin):
    ...
    def is_administrator(self):
        return self.Role.objects.filter(user=self, role_type=self.ModelRoles.ROLE_ADMINISTRATOR).exists() or self.is_superuser


    @cached_property
    def is_manager(self):
        return self.Role.objects.filter(user=self, role_type=self.ModelRoles.ROLE_MANAGER).exists()
    ...
    @cached_property
    def is_issuer(self):
        return Role.objects.filter(user=self, role_type=self.ModelRoles.ROLE_ISSUER).exists()
    # @property
    # def is_bot(self):
    ...
    @cached_property
    def is_judge(self):
        return self.Role.objects.filter(user=self, role_type=self.ModelRoles.ROLE_JUDGE).exists()

    @cached_property
    def is_organiser(self):
        return self.Role.objects.filter(user=self, role_type=self.ModelRoles.ROLE_ORGANISER).exists()

    @cached_property
    def is_competitor(self):
        '''Not adding everyone to role competitor anymore'''
        roles = self.Role.objects.filter(user=self)
        if roles.count() == 0:
            return True
    ...
    def has_role(self, role):
        ...
        if self.Role.objects.filter(user=self, role_type=role).exists():
            return True
    ...
    def user_roles(self, event_ref: str = None, descriptions: bool = False):
        ...
        non_event_roles = list(self.Role.objects.filter(user=self).values_list('role_type', flat=True))

        event_roles = []
        if event_ref:
            event_roles = list(
                self.EventRole.objects.filter(event_ref=event_ref, user=self, accepted=True).values_list('role_type',
                                                                                                    flat=True))
            # try:
    ...
    @classmethod
    def add_or_update_user(cls, email, passw='valegro', data=None):
        new = True
        try:
            user = cls.objects.get(email=email)
            new = False
        except cls.DoesNotExist:
            user = cls.objects.create_user(email=email, password=passw, **data)

        #
        # TODO: work out if this is established user or whether to silently update user

        # update user fields
        for f in cls._meta.get_fields():
            if f.name in data and not f.name in ('email', 'username', 'password', 'csrfmiddlewaretoken'):
                setattr(user, f.name, data[f.name])
